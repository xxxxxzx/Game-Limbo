...
namespace PlatformerPro
{
	...
	public class Stairs : Platform
	{
		...
		protected bool ShouldStartDescent(Character character, BasicRaycast collider) {
			...
			if (character.ActiveMovement is AirMovement && character.Velocity.y >= 0) return false;
			
			// Falling down = do stand on step
			if (character.ActiveMovement is AirMovement && character.Velocity.y < 0) return true;

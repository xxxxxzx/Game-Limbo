...
namespace PlatformerPro
{
	...
	public class PowerUpManager : GenericResponder, ICharacterReference
	{
		...
		virtual protected void Init()
		{
			...
			if (character == null) 
			{
				Debug.LogError ("A PowerUpManager must be the child of a character");
			}
			else
			{
		...
		{
			if (activePowerUps.Contains (resetResponse.type)) activePowerUps.Remove (resetResponse.type);
		...
		public bool Collect(string powerUpType) 
		{
		    ...
		    foreach (PowerUpResponse response in responses)
		    {
				...
				{
					activePowerUps.Add (response.type);

...
namespace PlatformerPro
{
	...
	public class SmoothCenteringZoneCamera : FixedZoneCamera
	{
		...
		/// </summary>
		protected List<CharacterLoader> characterLoaders;
		...
		override public void Init()
		{
			...
			if (characterLoader != null) characterLoaders = new List<CharacterLoader>(){characterLoader};
			else characterLoaders = CharacterLoader.GetCharacterLoaders();
			if (characterLoaders != null && characterLoaders.Count > 0)
			{
				foreach (CharacterLoader c in characterLoaders) c.CharacterLoaded += HandleCharacterLoaded;
			}

...
namespace PlatformerPro
{
	...
	public class EnemyMovement_VerticalSnapper : EnemyMovement
	{
		...
		/// <summary>
		/// Cached copy of starting yPos.
		/// </summary>
		protected float startingYPos;

		/// <summary>
		/// Cached copy of ending yPos.
		/// </summary>
		protected float endingYPos;
		...
		{
			this.enemy = enemy;
			startingYPos = enemy.transform.position.y;
			endingYPos = startingYPos + moveRange;
			if (retractSpeed < 0)
			{
		...
		override public bool DoMove()
		{
			...
			switch (state)
			{
				case SnapperState.CHARGING:
					...
					if (autoRetractAtTop && enemy.transform.position.y >= endingYPos) state = SnapperState.RETRACTING;
					break;
				case SnapperState.RETRACTING:
					enemy.Translate(0, TimeManager.FrameTime * -retractSpeed, true);
					if (enemy.transform.position.y <= startingYPos) state = SnapperState.IDLE;
					break;
			}
		...
		virtual protected void CheckForPlayer()
		{
			...
			if (hit.collider != null)
			{
				...
				{
					if (state == SnapperState.CHARGING && enemy.transform.position.y >= endingYPos) state = SnapperState.RETRACTING;
				}
			}
			else
			{
		...
		virtual public void OnDrawGizmos()
		{
			...
			Gizmos.DrawLine(transform.position,  transform.position + new Vector3(0, sightRange, 0));
			Gizmos.DrawLine(transform.position + new Vector3(0, sightRange, 0),  transform.position + new Vector3(arrowOffset, sightRange - arrowOffset, 0));
			Gizmos.DrawLine(transform.position + new Vector3(0, sightRange, 0),  transform.position + new Vector3(-arrowOffset, sightRange - arrowOffset, 0));
		}

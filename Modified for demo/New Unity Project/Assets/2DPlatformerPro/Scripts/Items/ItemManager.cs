...
namespace PlatformerPro
{
	...
	public class ItemManager : Persistable
	{
		...
		/// <param name="type">Type.</param>
		/// <param name="character">Character.</param>
		virtual protected void OnItemDepleted(ItemClass itemClass, string type, Character character)
		{
			if (ItemDepleted != null)
			{
				ItemDepleted(this, new ItemEventArgs(itemClass, type, 0, character));
			}
		}
		...
		{
			// If we collect something with an unknown type check to see if its stackable
			if (itemClass == ItemClass.NONE) 
			{
				foreach (StackableItemData item in collectibleItems) 
				{
					if (item.type == itemType) itemClass = ItemClass.STACKABLE;
				}
			}
		...
		virtual public int ConsumeItem(string itemType, int amount)
		{
			...
			if (itemData.ContainsKey(itemType)) 
			{
				...
				if (ItemCount(itemType) == 0) OnItemDepleted(itemData.TypeForKey(itemType), itemType, character);
				return actualAmount;
			}
			return 0;
		}

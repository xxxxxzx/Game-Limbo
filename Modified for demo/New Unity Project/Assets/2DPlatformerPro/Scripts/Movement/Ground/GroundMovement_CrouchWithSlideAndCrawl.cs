...
namespace PlatformerPro
{
	...
	public class GroundMovement_CrouchWithSlideAndCrawl : GroundMovement_CrouchWithSlide
	{
		...
		override public void DoMove()
		{
			...
			if (Mathf.Abs(character.Velocity.x) > crawlSpeed) isCrawling = false;
			...
			{
				if (character.Velocity.x < crawlSpeed && character.Input.HorizontalAxisDigital == 1)
				{
					character.SetVelocityX(crawlSpeed);
					isCrawling = true;
				}
				else if (character.Velocity.x > -crawlSpeed && character.Input.HorizontalAxisDigital == -1)
				{
					character.SetVelocityX(-crawlSpeed);
					isCrawling = true;
				}
